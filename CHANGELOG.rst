...
- Fixed ``RuntimeError: no running event loop`` traceback on ``kytosd`` shutdown in foreground mode, raised when the interactive shell teardown ran after the event loop had been closed.
- Fixed pidfile directory creation failing when a parent directory was missing or was created concurrently.
