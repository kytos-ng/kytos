...
The following table specifies expected **mef_eline** and **telemetry_int** actions when producing or handling certain events:
...
+----------------------------------+--------------------------------------------------------------------+-------------------------------------------------------+
| ``deployed``                     | remove and install flows (redeploy)                                | if requested INT, enable if first time or redeploy    |
+----------------------------------+--------------------------------------------------------------------+-------------------------------------------------------+
...
Major challenges to be aware when dealing with **mef_eline** events convergence: a) ensure failover fast convergence, and b) **telemetry_int** isn't differentiating which **mef_eline** path each flow belongs to. Whenever it's desirable that **telemetry_int** only perform a side effect on a subset of the flows, **mef_eline** should facilitate ideally by publishing upfront the set of the flows since **mef_eline** owns the follows and **telemetry_int** essentially follows. In general, the final outcome when handling these events is supposed to only add a few extra milliseconds on top of the existing 2023.2 **mef_eline** flows convergence:

- **mef_eline** will implement these new events: ``kytos/mef_eline.failover_link_down``, ``kytos/mef_eline.failover_old_path``, and ``kytos/mef_eline.failover_deployed``.
